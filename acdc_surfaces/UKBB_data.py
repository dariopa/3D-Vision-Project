...
def prepare_data(input_folder, input_GT_folder,output_file, mode, size, target_resolution, start_slice, end_slice,
    ...
    for folder in sorted(os.listdir(input_folder)):
        ...
        if os.path.isdir(folder_path):
            ...
            file = os.path.join(folder_path, 'tsc_sa_crop.nii.gz')


            fileGT = os.path.join(input_GT_folder, folder + '_sa_seg_allframes_resliced.nii.gz')
...
def load_and_maybe_process_data(input_folder,
    ...
    if not split_test_train:
        data_file_name = 'UKBB_data_%s_size_%s_res_%s_sl_%s_%s_onlytrain.hdf5' % (
        mode, size_str, res_str, start_slice, end_slice)
    else:
        data_file_name = 'UKBB_data_%s_size_%s_res_%s_sl_%s_%s.hdf5' % (mode, size_str, res_str, start_slice, end_slice)
    ...
    if not os.path.exists(data_file_path) or force_overwrite:
        ...
        prepare_data(input_folder, input_GT_folder, data_file_path, mode, size, target_resolution, start_slice, end_slice,
...
if __name__ == '__main__':
    input_folder = '/home/tothovak/work/CSR/Data/UKBB/'
    input_segmentation_folder = '/home/tothovak/work/CSR/Data/UKBB_data_segmentations'
    preprocessing_folder = 'preproc_data_UKBB_augmented'
    start_slice = 4
    end_slice = 4

    # d=load_and_maybe_process_data(input_folder, preprocessing_folder, '3D', (116,116,28), (2.5,2.5,5))
    d = load_and_maybe_process_data(input_folder, input_segmentation_folder, preprocessing_folder, '2D', (60, 60), (1.8, 1.8),
