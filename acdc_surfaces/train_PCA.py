...
if hasattr(exp_config, 'do_pca'):
    ...
    if (do_pca == True):
        PCA_U_file_path = os.path.join(sys_config.preproc_folder, 'full_UKBB_PCA_U.mat')
        PCA_mean_file_path = os.path.join(sys_config.preproc_folder, 'full_UKBB_PCA_mean.mat')
        PCA_sqrtsigma_file_path = os.path.join(sys_config.preproc_folder, 'full_UKBB_PCA_sqrtsigma.mat')
...
if hasattr(exp_config, 'multi_pixel_prediction'):
    multi_pixel = exp_config.multi_pixel_prediction
else:
    multi_pixel = False
...
if do_pca:
    ...
    log_dir_name = exp_config.experiment_name + str(exp_config.max_epochs) + 'Epoch_' + str(exp_config.learning_rate) + 'LR' + \
                   str(exp_config.batch_size) + 'Batch' + str(exp_config.keep_probability) + 'keep' + str(exp_config.weight_decay) + 'WD' + '_offset'
else:
    log_dir_name = exp_config.experiment_name + str(exp_config.max_epochs) + 'Epoch_' + str(exp_config.learning_rate) + 'LR' + \
...
def run_training(continue_run):
    ...
    with tf.Graph().as_default():
        ...
        (image_x, image_y) = exp_config.image_size

        image_tensor_shape = [exp_config.batch_size] + list(exp_config.image_size) + [1]
        if multi_pixel or multi_offset or do_pca:
            mask_tensor_shape = [exp_config.batch_size] + [image_x * image_y] + [exp_config.num_vertices * 2]
        else:
        ...
        if do_pca:
            [logits,_,_] = model.inference_multi_PCA(images_pl, exp_config, PCA_U_pl, PCA_mean_pl, PCA_sqrtsigma_pl, keep_prob=keep_prob_pl, training=training_pl)
        else:
            if multi_pixel:
                logits = model.inference_multi(images_pl, exp_config, keep_prob=keep_prob_pl, training=training_pl)
            elif multi_offset:
                logits = model.inference_multi_offset(images_pl, exp_config, multi_offset_matrix_pl, keep_prob=keep_prob_pl, training=training_pl)
            else:
            ...
            #                                                      augment_batch=exp_config.augment_batch)):
                ...
                if do_pca:
                    ...
                    feed_dict = {
                        images_pl: x,
                        labels_pl: y,
                        multi_offset_matrix_pl: multi_offset_matrix,
                        learning_rate_pl: curr_lr,
                        training_pl: True,
                        keep_prob_pl: exp_config.keep_probability
                }
...
def iterate_minibatches(images, labels, batch_size, augment_batch=False):
    ...
    for b_i in range(0,n_images,batch_size):
        ...
        if multi_pixel or multi_offset or do_pca: #FIXME
            y = np.repeat(y, image_x * image_y, axis = 1)
        else:
