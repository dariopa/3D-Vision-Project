...
########################################################################################
DATAFILE = '/home/tothovak/work/CSR/Code/acdc_segmenter/preproc_data/bbox_data_2D_size_212_212_res_1.36719_1.36719_sl_2_5.hdf5'
...
SESSION = '/home/tothovak/work/CSR/Code/acdc_segmenter/acdc_logdir/bounding_box5000Epoch_0.0001LR5Batch/'
...
def run_inference():
    ...
    with tf.Graph().as_default():
        ...
        if do_pca:
            logits = model.inferencePCA(images_pl, exp_config, PCA_U_pl, PCA_mean_pl, tf.constant(1,dtype=tf.float32), tf.constant(False, dtype=tf.bool))
        else:
        ...
        for i in range(images_test.shape[0]):
            ...
            if do_pca:
                pred = sess.run(logits, feed_dict={images_pl: np.expand_dims(images_test[i, :, :, :], axis=0), PCA_U_pl: PCA_U, PCA_mean_pl: PCA_mean})
            else:
            ...
            utils.overlay_bboxes(outImageBW, res, outGT, outOverlay, axis_limits)

            # compute precision measure
